...
name: htmx
description: >
  Designing and building hypermedia-driven web applications with htmx 2.x:
  choosing whether hypermedia fits, shaping server endpoints around UI needs,
  picking targets and swap strategies, updating content outside the target,
  the HX-* request/response header protocol, error and validation handling,
  scripting boundaries, and the security rules. MUST be invoked whenever htmx
  is used, added, reviewed, or debugged, whenever a page is built with
  `hx-get`/`hx-post`/`hx-swap`/`hx-target`/`hx-trigger`, and whenever deciding
  between htmx and a JavaScript SPA framework for a feature.
when_to_use: >
  ...
  here", "hx-swap-oob", "out of band", "HX-Trigger", "hx-boost", "should the
  server return HTML or JSON", "server-rendered partial", "template fragment",
  "my 422 response isn't swapping", "htmx 1 to 2 migration", "which htmx
  extension", "SSE with htmx", or when reviewing templates that carry `hx-*`
  attributes. Also use when a hypermedia-driven design is on the table under
  another name: HDA, MPA, "server-rendered with partials", Hotwire/Turbo,
  Unpoly, Datastar, fixi.
---
...
Two references sit alongside this file. Read them when you need them, not up
front:

- `${CLAUDE_SKILL_DIR}/references/reference.md`: every attribute, swap
  strategy, trigger modifier, header, event, config flag, and the core
  extensions. Consult it before writing an attribute you are not certain of.
- `${CLAUDE_SKILL_DIR}/references/patterns.md`: the canonical UX patterns
  (click-to-edit, active search, infinite scroll, inline validation, progress
  bar, delete row, modals, cascading selects) as working markup.
...
`this`) over sprinkling `id` attributes across the DOM. Hoist shared attributes
onto a common parent when it removes real repetition: htmx inherits most
attributes down the tree. That is a deliberate trade of Locality of Behaviour
for DRY, and it gets worse the further the attribute sits from the element it
governs. A `hx-target` on the enclosing `tbody` is fine; one three templates
away is not. Projects that dislike the trade entirely can set
`htmx.config.disableInheritance` and opt in with `hx-inherit`.

Keep an element's `id` stable across a swap and CSS transitions work with no
JavaScript: htmx copies old attribute values onto the new content, swaps, then
settles the new values ~20ms later. `htmx-swapping`, `htmx-settling`, and
`htmx-added` classes exist for fade-out and fade-in; `hx-swap="... swap:1s"`
lengthens the window for the animation to run.
...
1. **Expand the target.** Wrap the form and the table it feeds in one element
   ...
   are near each other.
2. **Out-of-band swaps.** Mark extra elements in the response with
   `hx-swap-oob`; they are swapped by `id` wherever they live. Good for
   piggy-backing a flash message or a counter onto another response.
3. **Events.** Return `HX-Trigger: newContact` and have the dependent element
...
4. **Path dependencies** via the `path-deps` extension. Interesting, but it
   fires on every matching request whether or not anything changed.
...
- Avoid `fetch()`/`XHR` returning JSON that you then render client-side. If a
  library needs that, wrap it as an **island** and connect it to the page with
  DOM events.
...
- `hx-on:click="..."` / `hx-on::after-request="..."` handle any event inline,
  including htmx's own. Attribute names are case-insensitive, so htmx events
  must be written kebab-case (`hx-on:htmx:before-request`), and `hx-on::` is
  shorthand for the `htmx:` prefix.
- Web components work well here, including Shadow DOM (call `htmx.process(root)`
  in `connectedCallback`; `host` and `global` reach outside the shadow root).

## The server contract

- `HX-Request: true` on every htmx request; `HX-Trigger`, `HX-Trigger-Name`,
  `HX-Target`, `HX-Current-URL`, `HX-Boosted`, `HX-Prompt` carry the rest.
- Responses can steer the client with `HX-Retarget`, `HX-Reswap`, `HX-Reselect`,
  `HX-Trigger[-After-Swap|-After-Settle]`, `HX-Push-Url`, `HX-Replace-Url`,
...
- `4xx`/`5xx` do **not** swap by default, and neither does `204`. A framework
  that returns `422` for a failed form will look like nothing happened. Fix it
  deliberately with `htmx.config.responseHandling`, the `response-targets`
  extension (`hx-target-404`, `hx-target-error`), or an `htmx:beforeSwap`
  handler, and say which you chose.
- `hx-push-url` obliges the server to serve that URL as a full page: users
  paste URLs, and a history cache miss re-requests it.
- Caching: if one URL returns a full page or a fragment depending on
  `HX-Request`, send `Vary: HX-Request` (or set `getCacheBusterParam`), and set
  `htmx.config.historyRestoreAsHxRequest` to `false` so restore requests are not
  mistaken for partial requests.
...
1. **Only call routes you control.** Relative URLs only. htmx inserts the
   response as live HTML; a third-party route can ship a `<script>`. htmx 2
   blocks cross-domain requests by default (`selfRequestsOnly`), so leave that
   alone.
...
3. **Put user content only inside HTML tags**, never inside `<script>` or
   ...
   raw HTML in `hx-disable`, which cannot be turned off by injected content.
...
Beyond that: `hx-headers` on `<html>` or `<body>` is the usual CSRF-token
vehicle (but `hx-boost` never replaces those tags, so a boosted app needs the
token somewhere that does get replaced); `hx-history="false"` keeps a sensitive
page out of `localStorage`; `htmx.config.allowEval=false` disables trigger
filters, `hx-on:`, and `js:` prefixes for a strict CSP.
...
swapped so new `head` styles and scripts are dropped (the `head-support`
extension fixes that), the global JS scope is not reset between "pages", and the
history cache is easy to get wrong.
...
Do not recite a version or an integrity hash from memory. Fetch
<https://htmx.org/docs/#installing> (or the npm/jsDelivr listing) for the current
`<script>` tag, and pin the version. Extensions ship separately from core as
`htmx-ext-<name>` on npm, except idiomorph, which is `idiomorph`. Load htmx with
a plain blocking `<script>` tag: `defer`, `type="module"`, and injecting it via
AJAX are all documented as unreliable.

Vendoring the file into the repo and checking it in is a well-supported option
here, since htmx has no runtime dependencies.

## Gotchas

Read `${CLAUDE_SKILL_DIR}/references/gotchas.md` before debugging "the swap
isn't happening" or "the event never fires". It collects the quirks that produce
silent, confusing failures: `body` always swapping `innerHTML`, `GET` not
including enclosing form values, OOB tags being stripped, `<tr>` needing a
`<template>` wrapper, `revealed` breaking under `overflow: scroll`,
`from:` selectors not re-evaluating, and the htmx 1 to 2 default changes.
